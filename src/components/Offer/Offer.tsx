...
const Offer = (props: Props) => {
  ...
  const amount = `${parseFloat(offer.min).toLocaleString()} — ${parseFloat(
    offer.max
  ).toLocaleString()}`;
  ...
  return (
    ...
    >
      ...
      <Stack
        ...
        justifyContent="space-between"
      >
        {label && <CardTitle sx={{ flex: 1, order: '1' }}>{label}</CardTitle>}
        {fromLabel && (
          <CardTitle sx={{ flex: 1, order: '2' }}>{fromLabel}</CardTitle>
        )}
      </Stack>
